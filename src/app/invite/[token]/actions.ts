...
export async function acceptEmployeeInvitation(formData:FormData){
  ...
  if(!invitation||invitation.email.toLowerCase()!==session.user.email.toLowerCase()||!(["manager","staff"] as string[]).includes(invitation.role)) redirect(`/invite/${encodeURIComponent(token)}?error=invalid`);
  await db.transaction(async tx=>{
    await tx.insert(memberships).values({organizationId:invitation.organizationId,storeId:invitation.storeId,userId:session.user.id,role:invitation.role});
