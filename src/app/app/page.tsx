...
export default async function AppPage(){
  ...
  return <main className="app-shell"><aside><div className="wordmark">VNDR <span>Hub</span></div><nav><Link className="active" href="/app">Overview</Link>{permits("pos:sell")&&<a>POS</a>}{permits("vendors:read")&&<Link href="/app/setup">Retail setup</Link>}{role==="vendor"&&<Link href="/vendor">Vendor portal</Link>}<a>Inventory</a>{permits("reports:read")&&<a>Reports</a>}{permits("team:read")&&<Link href="/app/team">Team</Link>}</nav></aside><section className="app-content"><header><div><p className="eyebrow">{role.replace("_"," ").toUpperCase()}</p><h1>{record.organization.name}</h1></div><SignOutButton/></header><div className="welcome-panel"><p className="eyebrow">YOUR ACCESS</p><h2>Welcome, {session.user.name}.</h2><p>{accessMessage}</p></div><div className="metric-grid"><article><span>Plan</span><b>{record.organization.plan}</b></article><article><span>Vendor allowance</span><b>{record.organization.plan==="unlimited"?"Unlimited":"40"}</b></article><article><span>Account role</span><b>{membership.role}</b></article></div></section></main>;
}
