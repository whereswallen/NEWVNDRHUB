...
const profileInput=z.object({organizationId:z.string().uuid(),storeId:z.string().uuid(),addressLine1:z.string().trim().min(2).max(160),city:z.string().trim().min(2).max(100),postalCode:z.string().trim().min(3).max(12),phone:z.string().trim().min(7).max(30),email:z.string().trim().toLowerCase().email(),receiptFooter:z.string().trim().max(300).optional()});

export async function saveStoreProfile(formData:FormData){const parsed=profileInput.safeParse(Object.fromEntries(formData));if(!parsed.success)redirect("/app/setup?error=profile");const {organizationId,storeId,...profile}=parsed.data;const {session}=await requireOrganizationPermission(organizationId,"store:manage");const [store]=await db.update(stores).set({...profile,updatedAt:new Date()}).where(and(eq(stores.id,storeId),eq(stores.organizationId,organizationId))).returning();if(!store)redirect("/app/setup?error=profile");await db.insert(auditLog).values({organizationId,storeId:store.id,actorUserId:session.user.id,action:"store.profile.updated",entityType:"store",entityId:store.id});revalidatePath("/app/setup")}
