...
export default async function BillingPage({searchParams}:{searchParams:Promise<{blocked?:string;checkout?:string;error?:string,synced?:string}>}){
  ...
  const {organization}=await requireCurrentOrganizationPermission("billing:manage",{allowBillingOnly:true});
  if(organization.status!=="active")redirect("/app");
  const [{count}]=await db.select({count:sql<number>`count(*)`}).from(stores).where(eq(stores.organizationId,organization.id));
  ...
  return <main className="management-shell"><header><div><p className="eyebrow">SUBSCRIPTION</p><h1>Billing and plan</h1><p className="muted">Subscription access is controlled by verified Stripe events.</p></div><Link className="quiet-button" href="/app">Back</Link></header>
    ...
    <section className="metric-grid"><article><span>Selected plan</span><b className="capitalize">{organization.plan}</b></article><article><span>Billing status</span><b className="capitalize">{organization.billingStatus}</b></article><article><span>Storefronts</span><b>{count}</b></article></section>
    <section className="management-card"><h2>Monthly subscription</h2><p>{organization.plan==="unlimited"?"$99 CAD":"$59 CAD"} plus ${(extraCost/100).toFixed(2)} CAD for additional storefronts, before applicable tax.</p>{organization.trialEndsAt&&<p className="muted">Trial ends {organization.trialEndsAt.toLocaleDateString("en-CA")}.</p>}
      {!configured?<p className="form-error">Stripe credentials and price IDs must be configured before live checkout.</p>:organization.stripeSubscriptionId?<div className="header-actions"><form action={openBillingPortal}><input type="hidden" name="organizationId" value={organization.id}/><button className="primary-button">Manage billing in Stripe</button></form><form action={syncStorefrontBilling}><input type="hidden" name="organizationId" value={organization.id}/><button className="quiet-button">Sync storefront billing</button></form></div>:<form action={startCheckout}><input type="hidden" name="organizationId" value={organization.id}/><button className="primary-button">Activate subscription</button></form>}
    </section></main>
}
