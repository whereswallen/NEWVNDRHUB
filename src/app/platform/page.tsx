...
export default async function PlatformPage(){
  ...
  return <main className="management-shell"><header><div><p className="eyebrow">VNDR HUB ADMIN</p><h1>Platform overview</h1><p className="muted">Signed in as {session.user.email}. This view spans every customer organization.</p></div><a className="quiet-button" href="/app">Customer app</a></header><section className="metric-grid"><article><span>Organizations</span><b>{customers.length}</b></article><article><span>Storefronts</span><b>{customers.reduce((sum,item)=>sum+Number(item.storeCount),0)}</b></article><article><span>Active vendors</span><b>{customers.reduce((sum,item)=>sum+Number(item.vendorCount),0)}</b></article></section><section className="management-card"><h2>Customer accounts</h2><div className="data-list">{customers.map(customer=><div key={customer.id}><span><b>{customer.name}</b><small>{customer.status}</small></span><span><b className="capitalize">{customer.plan}</b><small>{customer.storeCount} stores, {customer.vendorCount} vendors</small></span></div>)}</div></section></main>;
}
